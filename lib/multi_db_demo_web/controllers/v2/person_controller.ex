defmodule MultiDbDemoWeb.V2.PersonController do
  use MultiDbDemoWeb, :controller

  alias MultiDbDemo.V2.Persons
  alias MultiDbDemo.V2.Persons.Person

  action_fallback MultiDbDemoWeb.FallbackController
  ...
  def create(conn, %{"person" => person_params}) do
    with {:ok, %Person{} = person} <- Persons.create_person(person_params) do
      conn
      |> put_status(:created)
      |> put_resp_header("location", Routes.v2_person_path(conn, :show, person))
